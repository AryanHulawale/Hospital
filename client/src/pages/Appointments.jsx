...
const Appointments = () => {
    ...
    const displayedAppointments = appointments.filter(app => {
        ...
        const appDoctorUserId = app.doctor?._id || app.doctor;
        return appDoctorUserId?.toString() === filterDoctorId?.toString();
    });
            ...
            <div className="max-w-[1600px] mx-auto flex flex-col xl:flex-row gap-6">
                ...
                <div className="flex-1 flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-[#15232d] p-5 rounded-xl shadow-soft border border-[#e2e8f0] dark:border-[#1e2e38]">
                        ...
                        <button onClick={() => setIsModalOpen(true)} className="h-10 px-5 bg-[#0284c5] hover:bg-[#026aa0] text-white text-sm font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg">
                    ...
                    <div className="flex items-center gap-3 bg-white dark:bg-[#15232d] px-4 py-3 rounded-xl border border-[#e2e8f0] dark:border-[#1e2e38] shadow-sm">
                        ...
                        <div className="relative flex-1 max-w-xs">
                            <select value={filterDoctorId} onChange={(e) => setFilterDoctorId(e.target.value)} className="w-full h-9 pl-3 pr-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#1e2e38] text-sm font-bold text-[#101618] dark:text-white outline-none focus:ring-2 focus:ring-[#0284c5]/20 appearance-none cursor-pointer">
                                <option value="all">All Doctors</option>
                                {doctors.map(doc => (
                                    <option key={doc._id} value={doc.user?._id || doc.user}>Dr. {doc.name}</option>
                                ))}
                            </select>
                        ...
                        {filterDoctorId !== 'all' && (
                            <button onClick={() => setFilterDoctorId('all')} className="flex items-center gap-1.5 text-xs font-bold text-red-500 hover:bg-red-50 px-2 py-1.5 rounded-lg transition-colors">
                        ...
                        {displayedAppointments.length > 0 ? (
                            displayedAppointments.map((app) => (
                                <div key={app._id} className="flex gap-4 group animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div className={`flex-1 bg-white dark:bg-[#15232d] p-4 rounded-xl shadow-soft border border-[#e2e8f0] dark:border-[#1e2e38] border-l-4 hover:shadow-md transition-shadow ${app.status === 'Completed' ? 'border-l-emerald-500' : 'border-l-[#0284c5]'}`}>
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className={`text-[#101618] dark:text-white font-bold text-base ${app.status === 'Completed' ? 'text-slate-400 line-through' : ''}`}>
                                                        {app.patient?.fullname || app.patient?.name || 'Unknown Patient'}
                                                    </h3>
                                                    <div className="flex items-center gap-2 text-sm text-slate-500 mt-0.5">
                                                        <Stethoscope size={16} />
                                                        <span>{app.reason || 'General Consultation'} • <b>Dr. {app.doctorName}</b></span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex items-center justify-between md:justify-end gap-4">
                                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${app.status === 'Completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>
                            ...
                            <div className="text-center py-12 bg-white dark:bg-[#15232d] rounded-xl border-2 border-dashed border-slate-200">
                ...
                {/* Right Column: Calendar and Overview */}
                <div className="w-full xl:w-96 flex flex-col gap-6">
                    <div className="bg-white dark:bg-[#15232d] p-5 rounded-xl shadow-soft border border-[#e2e8f0]">
                        ...
                        <div className="grid grid-cols-7 text-center gap-y-1">
                            ...
                            {[...Array(daysInMonth)].map((_, i) => {
                                ...
                                return (
                                    <span key={dayNum} onClick={() => setSelectedDate(currentLoopDate)} className={`text-sm py-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-[#0284c5] text-white font-bold' : 'text-[#101618] dark:text-white hover:bg-slate-100'}`}>
                                        {dayNum}
                                    </span>
                                );
                            })}
                    ...
                    <div className="bg-white dark:bg-[#15232d] p-5 rounded-xl shadow-soft border border-[#e2e8f0] dark:border-[#1e2e38] flex flex-col gap-4">
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-[#1e2e38]">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">Today's Appointments</span>
                                    <span className="text-sm font-bold text-[#101618] dark:text-white">{displayedAppointments.length} Appointments</span>
                                </div>
                            </div>
                        </div>
                    </div>
